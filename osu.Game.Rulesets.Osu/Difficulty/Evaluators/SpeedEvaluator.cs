...
namespace osu.Game.Rulesets.Osu.Difficulty.Evaluators
{
    public static class SpeedEvaluator
    {
        ...
        public static double EvaluateDifficultyOf(DifficultyHitObject current, IReadOnlyList<Mod> mods, OsuDifficultyTuning tuning)
        {
            ...
            if (DifficultyCalculationUtils.MillisecondsToBPM(strainTime) > tuning.SpeedMinSpeedBonusBpm)
                speedBonus = 0.75 * Math.Pow((DifficultyCalculationUtils.BPMToMilliseconds(tuning.SpeedMinSpeedBonusBpm) - strainTime) / tuning.SpeedBalancingFactor, 2);

            double travelDistance = osuPrevObj?.TravelDistance ?? 0;
            double distance = travelDistance + osuCurrObj.MinimumJumpDistance;

            // Cap distance at single_spacing_threshold
            double singleSpacingThreshold = tuning.SpeedSingleSpacingThreshold;

            distance = Math.Min(distance, singleSpacingThreshold);

            // Max distance bonus is 1 * `distance_multiplier` at single_spacing_threshold
            double distanceBonus = Math.Pow(distance / singleSpacingThreshold, 3.95) * tuning.SpeedDistanceBonusScale;
